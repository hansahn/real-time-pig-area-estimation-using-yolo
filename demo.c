...
void *compute_size() {
    ///*Area*/
    area_img[Post_id] = threshold_otsu(in_img_resize[Post_id], 1); //원본이미지로 이진화, 1은 객체 흰색 2는 객체 검정
    area_img[Post_id] = openning(area_img[Post_id]);

    ///*Edge*/
    edge_img[Post_id] = edge(in_img_resize[Post_id], 1); //1. canny, 2. sobel + openning, 에지는 검정 출력

    ///*Merge */
    AND_img[Post_id] = AND_image(area_img[Post_id], edge_img[Post_id]); //면적영상과 에지 합성, 붙은돼지 떼기
